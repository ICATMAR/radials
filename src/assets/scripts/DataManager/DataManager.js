...
class DataManager {
  ...
  LANDMAKSURL = '/src/assets/img/LandMask_0_39.8_5_43.png';
  ...
  // Get antenna files
  getAntennaFiles(antennaID, timestamps, progressCallback) {

    let totalTmstsToLoad = timestamps.length;
    let tmstsLoaded = 0;
    ...
    for (let i = 0; i < timestamps.length; i++) {
      ...
      promises.push(window.FileManager.loadAntennaFiles(antennaID, timestamps[i]).then((res) => {
        ...
        if (res.length == 0) {
          ...
          let validResults = [];
          res.forEach(rr => {
            // There has been an error not related to file not being found
            if (rr.status == 'rejected' && !rr.reason.message.includes('File not found')) {
              console.error(rr.reason);
            } 
            // Error because file does not exist
            else if (rr.status == 'rejected' && rr.reason.message.includes('File not found')) {
              throw rr.reason; // File not found error
            }
            else {
